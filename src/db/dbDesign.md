...
### 1-to-1
- `move`
  - `move_requires_ptype`, with `ptype`
  - `move_requires_move`, with `move`

### 1-to-n
Every entity except `effect`, `status_ailment`, `usage_method`, `stat`, `description`, `version_group`, and `sprite` has `generation` as an identifying foreign key. For some 1-to-n relationships, we need to store additional data about the relationship, so we extract them to separate tables.
...
- `item`
  - `nature_power_ptype`, 1 type to n items
    - `power`, TINYINT
...
- `move`
  - `move_stat`, with `stat`
    ...
    - `target`, ENUM
    - `chance`, DECIMAL(5,2)
  - `move_causes_status_ailment`, with `status_ailment`
    - `chance`, DECIMAL(5,2)
  - `move_resists_status_ailment`, with `status_ailment`
...
- `ability`
  ...
  - `ability_stat`, with `stat`
    - `stage`, TINYINT
    - `multiplier`, DECIMAL(4,3)
  - `ability_causes_status_ailment`, with `status_ailment`
    - `probability`, DECIMAL(4,3)
  - `ability_resists_status_ailment`, with `status_ailment`
...
- `item`
  ...
  - `item_modifies_stat`, with `stat`
    - `stage`, TINYINT
    - `multiplier`, DECIMAL(4,3)
  - `item_causes_status_ailment`, with `status_ailment`
  - `item_resists_status_ailment`, with `status_ailment`
- `version_group`
